...
@observer
export default class GpuNodePodsCard extends React.Component {
  static propTypes = {
  ...
  componentDidUpdate(prevProps) {
    const { detail, details, isFederated, gpuSlice } = this.props
    const needUpdate = isFederated && !isEqual(details, prevProps.details)
    const sliceChanged = gpuSlice !== prevProps.gpuSlice

    if (detail !== prevProps.detail || needUpdate || sliceChanged) {
  ...
  fetchData = async ({ noMetrics, silent, ...params } = {}) => {
    ...
    const { limit, gpuSlice } = this.props

    silent && (this.store.list.silent = true)
    await this.store.fetchListGpuNode({
      limit,    
      gpuSlice, 
      ...this.state.params,
      ...params,      
