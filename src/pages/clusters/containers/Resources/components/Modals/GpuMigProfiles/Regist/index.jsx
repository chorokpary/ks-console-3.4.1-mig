...
import MigGpuTypeProfileRegist from './MigGpuTypeProfileRegist'

const RegistModal = props => {
  const store = props.store
  const rootStore = props.rootStore

  const existedNames = store.dataList.map(item => item.name)
                    ...
                    rules={[
                      ...
                      },
                      {
                        validator: (_, value) => {
                          ...
                          const name = `petasus-${value.trim()}`
                          const isDuplicated = existedNames.includes(name)

                          return isDuplicated
                            ? Promise.reject(
                                new Error(t('RESOURCES_DUPLICATE_NAME'))
                              )
                            : Promise.resolve()
                        },
                      },
...
export default inject('store', 'rootStore')(observer(RegistModal))
